...
class SS_GAE(SS_GAEg):
    def __init__(self, *args, **kwargs):
        super(SS_GAE, self).__init__(*args, **kwargs)

    def _get_vectors(self, graph):
        return super(SS_GAEg, self)._get_vectors(graph)  # ModelWithEmbeddings::_get_vectors

    def _get_embeddings(self, graph, **kwargs):
        self._ss_get_embeddings(graph, model_input.NODES)

class SS_GAE_deprecated(ModelWithEmbeddings):

    def __init__(self, output_dim=32, hiddens=None, max_degree=0, **kwargs):
        
        if hiddens is None:
            hiddens = [32]
        super(SS_GAE_deprecated, self).__init__(output_dim=output_dim, hiddens=hiddens, max_degree=max_degree, **kwargs)
